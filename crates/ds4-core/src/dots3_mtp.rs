...
impl crate::Session<'_> {
    pub(super) fn eval_dots3_argmax(
        &mut self,
        first: i32,
        max_tokens: i32,
        eos: i32,
        ...
        let n = unsafe {
            ds4_sys::ds4_bridge_dots3_trial(
                self.raw.as_ptr(),
                first,
                max_tokens,
                tokens.as_mut_ptr(),
        ...
        if n < 0 {
            // Native advances generation only after device state is touched.
            self.step_failed();
        ...
        if n as usize > TRIAL_CAP || n > max_tokens || tokens[0] != first {
            ...
            return Err(crate::Error {
                code: 1,
                message: "invalid dots3 trial result".into(),
            });
        }
        let n = n as usize;
        let Some(keep) = accepted_prefix(&tokens[..n], &target[..n], eos) else {
            ...
            return Err(crate::Error {
                code: 1,
                message: "invalid dots3 trial shape".into(),
            });
        };
        ...
        let rc = unsafe {
            ds4_sys::ds4_bridge_dots3_commit(
                self.raw.as_ptr(),
                keep as i32,
                err.as_mut_ptr().cast(),
                err.len(),
...
mod tests {
    ...
    fn acceptance_checks_trial_shape() {
        ...
        assert_eq!(accepted_prefix(&[10; 4], &[10; 4], 99), Some(4));
