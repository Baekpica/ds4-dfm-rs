...
constexpr int ROWS = TOKENS * USED, WIDTH = 2048, DEFAULT_THREADS = 128;
...
static_assert(WIDTH % (DEFAULT_THREADS * 4) == 0, "Production launch geometry");
...
int main(int argc, char **argv) {
    if (argc > 3) {
        std::fprintf(stderr, "usage: %s [repetitions=100] [threads=128|256|512]; PROBE_DUMP=full-output.bin\n", argv[0]);
        return 2;
    }
    const int reps = argc > 1 ? parse_reps(argv[1]) : DEFAULT_REPS;
    const int threads = argc > 2 ? parse_reps(argv[2]) : DEFAULT_THREADS;
    if (threads != 128 && threads != 256 && threads != 512) {
        std::fprintf(stderr, "threads must be 128, 256 or 512\n");
        return 2;
    }
    ...
    const dim3 grid(ROWS, WIDTH / (threads * 4));
    auto launch = [&] {
