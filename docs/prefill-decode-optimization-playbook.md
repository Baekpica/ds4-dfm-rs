...
frontier logits and token IDs were byte-identical. Optional NVTX changed median
prefill from 1247.86 to 1246.19 tok/s and decode from 25.22 to 25.11 tok/s
(-0.13% / -0.44%); this short smoke does not establish broader performance.
Scout found prefill `HOST / IDLE` (73.1% GPU coverage) and decode `DOMINANT KERNEL`
(`mul_mat_vec_q`, 89.0% coverage). Both range names and phase kernel totals were
checked against a separate Nsight export of the raw report.

Existing `speed-bench/` scripts remain valid for specialized server and
deep-context profiling.
