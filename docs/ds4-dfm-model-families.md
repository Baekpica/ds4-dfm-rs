...
| MiMo-V2.6-Flash-RL | `general.architecture=mimo2` | 48 trunk layers, 9 full-attention and 39 SWA-128, 256 experts top-8, three embedded dense MTP blocks. Embedded MTP is qualified when no DFlash file is loaded. DFlash is the external five-layer draft. Image, audio, and video are qualified. Qualified context 524288. Prefill chunk 4096. max_seqs 2 is omitted because the graph is serial. Media spans follow the 64MiB decode cap. 1M is not qualified. |
