...
- `DS4_INKLING_NO_IQ2_ALIGNED=1` restores 66-byte IQ2_XXS expert tiles.
  ...
  artifacts keep the raw path. Decode is unchanged.
...
- `DS4_EXAONE_PREFILL_CHUNK=N` sets the EXAONE-family prefill chunk. K2
  defaults to 1024 since the 2026-09-05 campaign (512 before); K-EXAONE
  keeps 512 until measured. Larger chunks fill the compact routed
  worklists better and halve the per-chunk launches; the batch workspace
  and sliding rings grow with the chunk. Numerics move at the fp level
  (router GEMM tiling, attention accumulation order).
...
- `DS4_SOLAR_FATTN_WS=0` restores the GQA-pair kernel. Default on: the
  ...
  through a two-stage `cp.async` ring and decode them into double-buffered
  half tiles while eight consumer warps run the same 16-key HMMA /
  online-softmax steps. Byte-identical. Uncapped 64K on GB10 drew ~105 W
  and froze the host; keep the 300–2200 MHz SM cap on that class of host.
